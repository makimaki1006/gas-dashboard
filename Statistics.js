...
function testStatistics() {
  ...
  // テストデータ（外れ値を含む給与データ - 万円）
  var salaryData = [25, 27, 28, 30, 32, 35, 38, 40, 45, 120]; // 最後は外れ値
  var salaryYen = salaryData.map(function(v) { return v * 10000; });

  console.log('\n[1] ブートストラップ信頼区間:');
  var bootstrap = getBootstrapConfidenceInterval(salaryYen, 2000);
  console.log('  95%信頼区間: ' + Math.round(bootstrap.lower / 10000) + '万円 〜 ' +
              Math.round(bootstrap.upper / 10000) + '万円');
  console.log('  ブートストラップ平均: ' + Math.round(bootstrap.bootstrapMean / 10000) + '万円');

  console.log('\n[2] 刈り込み平均:');
  var trimmed = getTrimmedMean(salaryYen, 0.1);
  console.log('  刈り込み平均: ' + Math.round(trimmed.trimmedMean / 10000) + '万円');
  console.log('  通常平均: ' + Math.round(trimmed.originalMean / 10000) + '万円');
  console.log('  除外データ数: ' + trimmed.removedCount);

  console.log('\n[3] 四分位統計:');
  var quartiles = getQuartileStats(salaryYen);
  console.log('  Q1: ' + Math.round(quartiles.q1 / 10000) + '万円');
  console.log('  Q2 (中央値): ' + Math.round(quartiles.q2 / 10000) + '万円');
  console.log('  Q3: ' + Math.round(quartiles.q3 / 10000) + '万円');
  console.log('  IQR: ' + Math.round(quartiles.iqr / 10000) + '万円');
  console.log('  外れ値: ' + quartiles.outlierCount + '件');

  console.log('\n[4] 包括的統計:');
  var enhanced = calculateEnhancedSalaryStatistics(salaryYen);
  console.log('  信頼性: ' + enhanced.reliabilityLabel);

  console.log('\n[5] 表示用フォーマット:');
  var formatted = formatStatisticsForDisplay(enhanced);
  console.log('  サマリー: ' + formatted.summary);
  if (formatted.outlierWarning) {
    console.log('  警告: ' + formatted.outlierWarning);
  }
