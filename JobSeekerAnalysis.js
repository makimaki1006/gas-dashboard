 ...
 * テスト関数
 */
function testJobSeekerAnalysis() {
  console.log('=== 求職者視点分析テスト ===');

  // データ取得
  var aggregation = DataLayer.getAggregation();
  if (!aggregation || !aggregation.rawRecords) {
    console.log('データがありません');
    return;
  }

  var parsedData = aggregation.rawRecords;
  console.log('総レコード数: ' + parsedData.length);

  // 分析実行
  var result = analyzeJobSeekerPerspective(parsedData);

  // 結果出力
  console.log('\n[1] 給与レンジの心理的解釈:');
  if (result.salaryRangePerception && result.salaryRangePerception.hasData) {
    console.log('  ' + result.salaryRangePerception.interpretation);
  }

  console.log('\n[2] 新着求人の特徴:');
  if (result.newListingsAnalysis && result.newListingsAnalysis.hasData) {
    console.log('  ' + result.newListingsAnalysis.interpretation);
  }

  console.log('\n[3] 未経験可タグの給与差:');
  if (result.inexperiencedTagAnalysis && result.inexperiencedTagAnalysis.hasData) {
    console.log('  ' + result.inexperiencedTagAnalysis.interpretation);
  }

  console.log('\n[4] 暗黙の相場観:');
  if (result.implicitMarketRate && result.implicitMarketRate.hasData) {
    console.log('  ' + result.implicitMarketRate.interpretation);
  }
